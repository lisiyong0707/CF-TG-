/**
 * Domain Manager Pro (融合版) - V2 增强版
 * 后端：增加 WHOIS解析 + 多通道通知 + 批量删除 + 自定义勾选同步逻辑
 * 前端：赛博狸花猫AI + 灵活同步选择器 + 【新增】DNS DoH 诊断 + 【新增】赛博算卦估值
 */
...
async function syncCF(b, env) {
  ...
  for (const d of r.domains) {
    if (selectedSet && !selectedSet.has(d.name)) {
      skipped++;
      continue;
    }
...
.cp-btn { cursor:pointer; opacity:0.4; transition:0.2s; font-size:16px; margin-left:6px; user-select: none; display: inline-flex; align-items:center; justify-content:center; }
...
.sync-item {
  ...
  cursor: pointer; transition: 0.2s; border-radius: 8px; margin-bottom: 4px;
}
.sync-item:hover { background: #f8fafc; }
</style>
</head>
<body>
...
<div id="cat-container">
  ...
  <div id="cat-chat">
    ...
    <div style="display:flex; gap:5px; padding:8px 12px; overflow-x:auto; border-bottom:1px solid #e2e8f0; background:#f8fafc;" id="chat-prompts">
       ...
       <button class="b bx" style="cursor:pointer; border:1px solid #e2e8f0; white-space:nowrap;" onclick="sendQuick('算卦')">🔮 算一卦</button>
       <button class="b bx" style="cursor:pointer; border:1px solid #e2e8f0; white-space:nowrap;" onclick="sendQuick('诊断')">🌐 DNS诊断</button>
...
<div class="mo" id="sm"><div class="md">
  ...
  <div id="sbody" style="display:none">
    <div id="ssum" style="margin:20px 0; font-weight:700; color:var(--primary); line-height:1.6"></div>
    <div id="slist" style="max-height:280px; overflow-y:auto; border:2px solid #f1f5f9; border-radius:12px; background:#fff; padding:5px;"></div>
    <div class="fg" style="margin-top:20px">
      <label class="fl">同步覆写模式 (针对已存在的记录)</label>
      <select id="smod" class="fsel"><option value="new">仅导入新增域名资产 (跳过已有)</option><option value="all">完全同步 (覆写更新已有的到期日)</option></select>
    </div>
  </div>
...
function generateAiReply(query) {
  ...
  if (query.indexOf('算') > -1 || query.indexOf('卦') > -1 || query.indexOf('估值') > -1) {
    return "点击域名列表旁边小小的 🔮 图标，本喵就可以施展赛博算命术，为你评估这个域名的吉凶和身价哦喵！快去试试！";
  }
  if (query.indexOf('测') > -1 || query.indexOf('dns') > -1 || query.indexOf('诊断') > -1) {
    return "想知道域名的解析IP？点击域名旁边的小地球 🌐 图标，本喵会直接呼叫 Cloudflare DoH 接口帮你一键看穿它的真实位置喵！";
  }
...
// 趣味功能：赛博算卦
function appraiseDomain(domain) {
  var hash = domain.split('').reduce(function(a, b) { a = ((a << 5) - a) + b.charCodeAt(0); return a & a }, 0);
  var val = Math.abs(hash) % 888888;
  var words = ["极品靓号！", "大吉大利喵！", "有点潜力~", "适合传家喵", "平平无奇...", "风水不佳喵", "这域名有股鱼腥味~"];
  var word = words[Math.abs(hash) % words.length];
  showCatMsg('猫猫算卦 🔮【' + domain + '】预估灵力值: ¥' + val + '，' + word);
}

// 实用功能：DNS 诊断
function checkDNS(domain) {
  showCatMsg('正在通过 DoH 诊断【' + domain + '】的 A 记录... 🌐');
  fetch('https://cloudflare-dns.com/dns-query?name=' + domain + '&type=A', { headers: { 'accept': 'application/dns-json' } })
    .then(function(r){return r.json()}).then(function(data){
      var ip = data.Answer ? data.Answer.map(function(a){return a.data}).join(', ') : '暂无 A 记录';
      setTimeout(function(){ showCatMsg('【' + domain + '】解析结果: ' + ip); }, 1000);
  }).catch(function(){ showCatMsg('测不通喵！网络受阻或无记录！', true); });
}
...
function renderD(list) {
  ...
  tb.innerHTML = list.map(function(d) {
    ...
    return '<tr><td style="padding-right:10px;"><input type="checkbox" class="dsel" value="' + d.id + '" onchange="updateSel()"></td><td>'
      + '<div class="dn">' + d.name + ' <span class="cp-btn" id="cp-'+d.id+'" onclick="copyTxt(\''+d.name+'\', \''+d.id+'\')" title="复制">📋</span> <span class="cp-btn" onclick="appraiseDomain(\''+d.name+'\')" title="AI算卦估值">🔮</span> <span class="cp-btn" onclick="checkDNS(\''+d.name+'\')" title="诊断A记录">🌐</span></div><div style="margin-top:6px">' + srcBadge + '</div>' + pb + '</td>'
...
function renderCF() {
  ...
  g.innerHTML = CF.map(function(a) {
    return '<div class="sc" style="text-align:left; border-top:4px solid #f6821f"><div><strong>☁️ ' + a.name + '</strong></div><div style="font-size:12px;color:#94a3b8;margin:5px 0 15px">' + a.cfAccountName + '<br><span style="color:#f6821f; font-weight:700">承载 ' + (cnt[a.id]||0) + ' 个节点</span></div><div style="display:flex;gap:8px"><button class="btn bp" style="padding:8px 18px" onclick="openSync(\'' + a.id + '\')">↻ 云端同步</button><button class="btn bs" style="padding:8px 18px; color:#ef4444" onclick="delCF(\'' + a.id + '\')">断开</button></div></div>';
  }).join('');
}
...
function openSync(cfId) {
  ...
  post('/api/cf-preview', { cfAccountId: cfId }).then(function(r) {
    ...
    document.getElementById('ssum').innerHTML = '扫描到 <strong>' + r.total + '</strong> 个网关节点，包含 <strong>' + r.newCount + '</strong> 个新节点。<br>' +
      '<label style="cursor:pointer; margin-top:8px; display:inline-flex; align-items:center; gap:5px; font-size:13px; font-weight:bold">' +
      '<input type="checkbox" id="sync-sel-all" checked onchange="toggleSyncAll(this)" style="width:14px;height:14px;accent-color:var(--primary)">全选 / 取消全选</label>';
    
    document.getElementById('slist').innerHTML = r.domains.map(function(d){ 
      return '<label class="sync-item"><div style="display:flex; align-items:center; gap:10px">' +
             '<input type="checkbox" class="sync-cb" value="' + d.name + '" checked style="accent-color:var(--primary)">' +
             '<span style="font-weight:700">' + d.name + (d.exists ? ' <span class="b bx" style="font-size:10px">已存在</span>' : '') + '</span></div>' +
             '<span style="color:#94a3b8; font-family:monospace">' + (d.expiryDate || '永久有效') + '</span></label>'; 
    }).join('');
  });
}
function toggleSyncAll(el) { document.querySelectorAll('.sync-cb').forEach(function(cb) { cb.checked = el.checked; }); }

function doSync() { 
  var sel = Array.from(document.querySelectorAll('.sync-cb:checked')).map(function(cb){ return cb.value; });
  if(sel.length===0) return toast('请至少勾选一个节点', 'e');
  
  var btn = document.getElementById('sbtn'); btn.textContent = '数据传输中...'; btn.disabled = true;
  post('/api/cf-sync', { cfAccountId: curCFId, mode: gval('smod'), selectedDomains: sel }).then(function(r) { 
    btn.textContent = '确认执行同步'; btn.disabled = false; 
    if (r) { 
      closeM('sm'); 
      toast('同步完成！新增 ' + r.added + '，更新 ' + r.updated + '，跳过 ' + r.skipped); 
      loadD(); loadStats(); 
      showCatMsg('云端数据精准拉取完毕喵~'); 
    } 
  }); 
}

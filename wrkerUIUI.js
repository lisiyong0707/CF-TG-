 ...
 * 后端：增加 WHOIS解析 + 多通道通知(微信/Bark/TG/Email) + 批量删除 + TG容错机制(支持特殊字符安全转义)
 * 前端：智能解析 + 赛博狸花猫AI(段位养成+自由奔跑+对话问答+快捷指令) + 数据导出CSV
 */
  ...
  <div id="p2" class="page">
    <div class="tw" style="padding:50px; max-width:700px; margin:0 auto;">
      ...
      <div style="display:flex; gap:15px; margin-top:20px;">
        <!-- 🚀 修复点 3：测试按钮改为自动调用合并逻辑，保证测的是当前填写的最新参数 -->
...
<div class="mo" id="bm"><div class="md">
  ...
  <div style="font-size:13px; color:#64748b; margin-bottom:15px; background:#f8fafc; padding:15px; border-radius:12px; line-height: 1.6;">
    💡 支持直接粘贴包含域名的多行文本，智能提取域名及到期时间（支持永久有效格式）。
  </div>
  <div class="fg"><label class="fl">默认注册商</label><input class="fi" id="breg" placeholder="例如：批量录入标签"></div>
  <textarea class="fta" id="btext" rows="10" placeholder="粘贴列表文本，如：&#10;test.com 2026-05-12&#10;my.net 永久有效" style="font-family:monospace; line-height: 1.5;"></textarea>
...
function generateAiReply(query) {
  ...
  return "喵？我是一个本地执行的赛博狸花猫管家。你可以问我关于【到期资产】、【节点总数】的问题，或者让我去【跑一跑】哦~";
}
...
// 自动拦截并保存：为了杜绝忘记保存造成的报错，现在测试按钮会自动触发保存
...
function openDM(d) {
  ...
  val('dexp', d ? d.expiryDate||'' : ''); val('dacc', d ? d.accountId||'' : ''); val('dnotes', d ? d.notes||'' : '');
...
async function saveBulk() {
  var txt = gval('btext'), defReg = gval('breg').trim(); if(!txt.trim()) return toast('数据源为空', 'e');
  var lines = txt.split('\n'), btn = document.getElementById('bbtn'); btn.innerHTML = '运算导入中...'; btn.disabled = true; var added = 0;
  for(var i=0; i<lines.length; i++) {
    var line = lines[i].trim(); if(!line) continue;
    var domMatch = line.match(/([a-zA-Z0-9\u4e00-\u9fa5-]+\.)+[a-zA-Z]{2,}/); if(!domMatch) continue; 
    var domain = domMatch[0].toLowerCase(), dates = (line.match(/\d{4}[-/年]\d{1,2}[-/月]\d{1,2}日?/g) ||[]).map(function(d){ 
      var p = d.replace(/[年月\/]/g, '-').replace(/日/g, '').split('-'); return p[0] + '-' + (p[1].length===1?'0'+p[1]:p[1]) + '-' + (p[2].length===1?'0'+p[2]:p[2]);
    });
    var expDate = '', regDate = '', isPermanent = /永久|永不过期/.test(line);
    if (isPermanent) { expDate = '2099-12-31'; if (dates.length > 0) regDate = dates[0]; } else { if (dates.length >= 2) { regDate = dates[0]; expDate = dates[1]; } else if (dates.length === 1) { expDate = dates[0]; } }
    if(!expDate) continue; 
    await post('/api/domains', { name: domain, expiryDate: expDate, registeredAt: regDate, registrar: defReg || '未知/批量导入', autoRenew: false, source: 'bulk' }); added++;
  }
  btn.innerHTML = '智能提取并导入'; btn.disabled = false; closeM('bm'); toast('成功解析并导入 ' + added + ' 个极客资产！'); loadD(); loadStats();

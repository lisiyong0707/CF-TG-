  ...
  try {
    ...
    if (zones.success) {
      ...
      for (const z of zones.result || []) {
        if (!s.has(z.name)) {
          const regAt = (z.created_on || '').split('T')[0];
          let expAt = '';
          if (regAt) {
            // 将到期时间默认设置为绑定时间的 1 年后
            const dObj = new Date(regAt);
            dObj.setFullYear(dObj.getFullYear() + 1);
            expAt = dObj.toISOString().split('T')[0];
          } else {
            const dObj = new Date();
            dObj.setFullYear(dObj.getFullYear() + 1);
            expAt = dObj.toISOString().split('T')[0];
          }
          out.push({ name: z.name, registeredAt: regAt, expiryDate: expAt, autoRenew: false, source: 'cf_zone' });
        }
      }
    }
...
const titlePool = ["让资产管理，自然呼吸", "种下一颗域名，收获一片森林", "你的数字花园，生机盎然", "告别吃灰，域名不再流浪", "赛博空间，也能长出绿叶", "极客与自然，完美的交响"];
function changeTitle() {
  const el = document.getElementById('hero-title');
  el.style.opacity = 0; el.style.transform = 'scale(0.95)';
  setTimeout(function() { el.innerText = titlePool[Math.floor(Math.random() * titlePool.length)]; el.style.opacity = 1; el.style.transform = 'scale(1)'; }, 300);
}
...
function parseBulk() {
  ...
  document.getElementById('bm-step2').style.display = 'flex';
}

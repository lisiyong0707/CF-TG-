/**
 * Domain Manager  (带TG快捷续费功能 - 自定义提醒天数版)
 */
...
async function route(request, env, path) {
  ...
  if (path === '/api/check' && m === 'POST') {
    ...
    if (!c.botToken || !c.chatId) {
      return ok({ error: '请先填写并保存 Bot Token 和 Chat ID' }, 400);
    }
    await tg(c.botToken, c.chatId, '✅ *测试成功*\n\n你的域名管理系统已成功连接到 Telegram！\n你可以随时向我发送 `/domains` 查看所有域名。');
    await env.KV.delete('last_check');
    await dailyCheck(env);
    return ok({ ok: true, msg: '测试消息已发送，请查看 TG' });
  }
...
async function handleWebhook(request, env) {
  ...
  if (txt === '/start') {
    await tg(c.botToken, cid, '🌐 *域名管理机器人*\n\n你的 Chat ID: `' + cid + '`\n\n/domains — 所有域名\n/expiring — 即将到期\n/check — 立即检查');
  } 
  else if (txt === '/domains') {
    const list = await kget(env, 'domains');
    ...
    if (!exp.length) {
      await tg(c.botToken, cid, '✅ 30天内无到期');
    } else {
      const buttons = [];
      ...
      await tg(c.botToken, cid, '⏰ *30天内到期*\n\n' + lines, buttons.length ? { inline_keyboard: buttons } : null);
    }
  } 
  else if (txt === '/check') { 
    await dailyCheck(env); 
    await tg(c.botToken, cid, '✅ 检查完成'); 
  }
  return ok({ ok: true });
}
